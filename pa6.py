    ...
    def change_combinations(total, combination, index):
        if total == 0:
            ...
            for i in range(index, len(coins)):
                if total - coins[i] >= 0:
                    change_combinations(total - coins[i], combination + [coins[i]], i)
...
class DTree:
    def __init__(self, variable, threshold, lessequal, greater, outcome):
        if outcome is None:
            if any([arg is None for arg in [variable, threshold, lessequal, greater]]):
                raise ValueError('If outcome is none, other variables cannot be none')
        else:
            if any([arg is not None for arg in [variable, threshold, lessequal, greater]]):
                raise ValueError('If outcome is not none, all first four arguments are none')
    ...
    def no_repeats(self):
        def repeat_tracker(self, vars_l):
            ...
            return repeat_tracker(self.lessequal, vars_l) and repeat_tracker(self.greater, vars_l)
        vars_l = []
